  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
      <label className="block">
        ...
        {errors.phone && (
          <span className="mt-2 block text-xs text-red-500">{errors.phone.message}</span>
        )}
      </label>

      {isError && (
        <p className="text-xs text-red-500">ارسال کد ناموفق بود، دوباره تلاش کنید.</p>
      )}
      ...
      >
        {isPending ? "در حال ارسال..." : "دریافت کد تایید"} <ArrowLeft size={17} />
      </button>
